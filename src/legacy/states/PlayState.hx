...
import legacy.states.menus.*;

import legacy.objects.Note;
import legacy.objects.Character;
import legacy.objects.Boyfriend;
import legacy.objects.DialogueBox;
import legacy.objects.HealthIcon;
import legacy.objects.background.*;
...
class PlayState extends MusicBeatState
{
	...
	var camDisplaceOffset:Float = 12.0;
	var camDisplaceOffsets:Array<Array<Float>>;
	var camResetTimer:FlxTimer;
	...
	override public function create()
	{
		...
		camDisplaceOffsets = [
			[-camDisplaceOffset, 0.0],
			[0.0, camDisplaceOffset],
			[0.0, -camDisplaceOffset],
			[camDisplaceOffset, 0.0]
		];
	...
	function startSong():Void
	{
		...
		@:privateAccess
		if (!paused) FlxG.sound.playMusic(_inst._sound);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		_inst = FlxG.sound.load('assets/music/' + curSong + '_Inst' + TitleState.soundExt);
	...
	{
		if (!FlxG.sound.music.playing || !SONG.needsVoices)  return;
		if (Math.abs(vocals.time - FlxG.sound.music.time) < 5.0) return;
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.justPressed.SEVEN)
			FlxG.switchState(new legacy.states.editors.ChartingState());

		iconP1.setGraphicSize(FlxMath.lerp(150, iconP1.width, Math.exp(-elapsed * 30.0)));
		iconP2.setGraphicSize(FlxMath.lerp(150, iconP2.width, Math.exp(-elapsed * 30.0)));
		...
		else
		{
			...
			{
				if (Conductor.lastSongPos != FlxG.sound.music.time)
				{
					Conductor.lastSongPos = FlxG.sound.music.time;
					Conductor.songPositionResync = 0.0;
				}
				else
					Conductor.songPositionResync += elapsed * 1000;

				Conductor.songPosition = Conductor.lastSongPos + Conductor.songPositionResync;
			}
			resyncVocals();
		}
		...
		if (camZooming)
		{
			FlxG.camera.zoom = FlxMath.lerp(defaultCamZoom, FlxG.camera.zoom, Math.exp(-elapsed * 9.0));
			camHUD.zoom = FlxMath.lerp(1, camHUD.zoom, Math.exp(-elapsed * 9.0));
		}
		...
		while (unspawnNotes.length != 0 && unspawnNotes[0] != null && unspawnNotes[0].strumTime - Conductor.songPosition < 1500.0 / SONG.speed)
		{
			if (unspawnNotes[0].strumTime - Conductor.songPosition > 1500.0 / SONG.speed)
				break;
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					moveCamDirection(daNote.noteData);
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		var daRating:String = '';
		var ratings:Array<String> = ['sick', 'good', 'bad', 'shit'];
		var windows:Array<Float> = [45.0, 90.0, 135.0, 166.0];

		function findRating(diff:Float = -1):String
		{
	...
	function moveCamDirection(noteData:Int):Void
	{
		FlxG.camera.targetOffset.set(camDisplaceOffsets[noteData][0], camDisplaceOffsets[noteData][1]);
		if (camResetTimer != null) camResetTimer.cancel();
		camResetTimer = new FlxTimer().start(Conductor.crochet * 0.001, _ -> FlxG.camera.targetOffset.set(0, 0));
	}
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			moveCamDirection(note.noteData);
